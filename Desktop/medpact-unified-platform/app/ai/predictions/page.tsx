...
export default function AIPredictionsPage() {
  ...
  const generatePredictions = async () => {
    setLoading(true);
    try {
      // In production, fetch actual contract IDs from user's organization
      const mockContractId = 'contract-123';
      const result = await PredictionEngine.predictContractRenewal(mockContractId);
            ...
            <Card>
              ...
              <CardContent>
                <div className="h-64 flex items-end justify-around gap-2">
                  {[2400, 2520, 2650, 2780, 2850, 2950].map((value, idx) => (
